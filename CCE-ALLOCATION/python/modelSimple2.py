...
def modelSimple2(numberOfUsers, R, numberOfSubframes, graph):
    try:
        if not graph:        
            f = open("output/cce_output_simple2_" + str(R) + ".txt", "w")
        solutionStatistics = SolutionStatistics()

        if R == 20:
            numberOfUsers = min(numberOfUsers, 10)
        ...
        for frame in range(numberOfSubframes):
            ...
            y = m.addVars(range(R), vtype=GRB.BINARY, name="y")

            # Set objective
            m.setObjective(quicksum([y[j] for j in range(R)]), GRB.MAXIMIZE)
            ...
            m.addConstrs((quicksum([x[(i, t)] for i in userRange for t in range(max(0, j - users[i].size + 1), j + 1)]) == y[j] \
