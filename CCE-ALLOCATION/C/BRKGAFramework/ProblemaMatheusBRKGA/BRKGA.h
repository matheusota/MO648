...
public:
    ...
    void inject_solution(std::vector<double> sol, int population, int position);
...
// Allow injecting a start solution
template< class Decoder, class RNG >
...
//    bestest_chromosome = sol;
//     printf("bestest_fitness\n" );
//     for(int i = 0; i< bestest_chromosome.size(); i++)
//       printf("%lf , ", bestest_chromosome[i]);
}
