...
void SchedulingModel2::execute(vector<User> &users2, int R, int numberUsers, Measures &measures){
    // copy users
    ...
    model.set(GRB_StringAttr_ModelName, "CCE_SCH2"); // gives a name to the problem
    ...
    for(int j = 0; j < R; j++)
        y[j] = model.addVar(0.0, 1.0, -1, GRB_BINARY, "y[" + to_string(j) + "]");
